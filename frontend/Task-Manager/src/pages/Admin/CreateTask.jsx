...
const CreateTask = () => {
  ...
  const handleSubmit = async ()=>{
       ...
       if (!taskData.description.trim()){
        setError("Decription is required.")
        return;
       }
  ...
  const deleteTask = async ()=>{
       try{
        ...
        console.error("Errro deleting tasks",error.response?.message|| error.message);
       }
  };
    ...
    <div className='mt-5'>
       <div className='grid grid-cols-1 md:grid-cols-4 mt-4'>
         <div className='form-card col-span-3'>
           ...
           <div className='mt-3'>
              <label className='text-xs font-medium text-slate-600'>Add Attachements</label>

              <AddAttachementsInput
                 attachements={taskData?.attachements}
                 setAttachements = {(value)=>handleValueChange("attachements", value)}
              />
           </div>

           {error && (
            <p className='text-xs font-mediumte text-red-500 mt-5'>{error}</p>
           )}
